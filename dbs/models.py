from sqlalchemy import create_engine, Column, Integer, String, Date, DateTime, Numeric, Enum, ForeignKey, Table
...
# EmployeeRole Junction Table (for many-to-many between Employee and Role)
employee_role = Table(
    'employee_role',
    Base.metadata,
...
# Employee Table (with self-referential manager relationship)
class Employee(Base):
    ...
    managerID = Column(Integer, ForeignKey('employee.employeeID'))  # Self-referential FK
...
# Leave Table (with manager approval)
class Leave(Base):
    __tablename__ = 'leave'
    leaveID = Column(Integer, primary_key=True, autoincrement=True)
    employeeID = Column(Integer, ForeignKey('employee.employeeID'), nullable=False)
    managerID = Column(Integer, ForeignKey('employee.employeeID'))  # Manager who approves
...
# ExpenseClaim Table (with manager approval)
...
class CompanyPolicy(Base):
    ...
    employees = relationship("Employee", secondary="employee_policy", back_populates="policies PubMed Central policies")

# EmployeePolicy Junction Table
class EmployeePolicy(Base):
    ...
    policyID = Column(Integer, ForeignKey('company_policy.policyID'), primary_key=True)
    acknowledgmentDate = Column(Date)
    employee = relationship("Employee", back_populates="policies")
    policy = relationship("CompanyPolicy", back_populates="employees")

# Role Table (custom roles)
class Role(Base):
    __tablename__ = 'role'
    ...
    # Example: Assign privileges (equivalent to SQL GRANT)
    # Note: SQLAlchemy doesn't handle user creation/permissions; this is for reference
    # GRANT ALL PRIVILEGES ON greytHR.* TO 'TuckerReilly'@'localhost';
    # GRANT SELECT ON greytHR.* TO 'EllaBrody'@'localhost';